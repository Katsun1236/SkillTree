...
export default function Login() {
  ...
  // Fonction pour la connexion classique par email
  ...
  // Fonction pour la connexion via Discord
  const signInWithDiscord = async () => {
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'discord',
      options: {
        // Redirige vers ton site Netlify une fois la connexion Discord validée
        redirectTo: 'https://skilltree-bimpactstudio.netlify.app/', 
      },
    });
    if (error) console.error("Erreur Discord:", error.message);
  };
    ...
    <div className="h-full w-full flex items-center justify-center bg-slate-50">
      <div className="w-full max-w-md p-8 bg-white border border-slate-200 rounded-2xl shadow-xl">
        ...
        <button 
          ...
          type="button"
          className="w-full bg-[#5865F2] text-white p-3 rounded-lg font-bold flex items-center justify-center gap-3 hover:bg-[#4752C4] transition shadow-md mt-2"
        >
          {/* Logo officiel Discord en SVG */}
